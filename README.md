...
这个 skill 就是为了解决这些问题而生的。它把写毕设的整个流程标准化为 9 个阶段，每个阶段都有明确的操作指南和自动化工具。
...
aigc-reduce 专门做 AIGC 降重，ug-thesis-writing 覆盖毕设写作的全流程，降重只是其中一个阶段。ug-thesis-writing 会调用 aigc-reduce 来完成降重工作。

### Q: 支持 LaTeX 论文吗？
本 skill 专注于 Word (.docx) 格式，这是国内本科毕设最常用的格式。如果你用 LaTeX 写论文，建议使用 ar-paper-writing-section 和 ar-latex-formatting 等技能。

### Q: 支持哪些学校？
理论上支持所有中国高校的本科毕设。格式检查通过配置文件适配，你可以根据学校的格式要求修改 `format.json`。
